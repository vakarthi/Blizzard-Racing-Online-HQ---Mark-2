...
const navItems: NavItem[] = [
  ...
  { path: 'leads', name: 'Leads', icon: <BriefcaseIcon className="w-5 h-5" />, roles: [UserRole.Manager, UserRole.Marketing] },
  { path: 'socials', name: 'Socials', icon: <MessageSquareIcon className="w-5 h-5" />, roles: [UserRole.Manager, UserRole.SocialsDesigner, UserRole.Marketing] },
  { path: 'toolbox', name: 'Toolbox', icon: <WrenchIcon className="w-5 h-5" />, roles: [UserRole.Manager, UserRole.ManufacturingEngineer, UserRole.DesignEngineer] },
  { path: 'portal-editor', name: 'Portal Editor', icon: <EditIcon className="w-5 h-5" />, roles: [UserRole.Manager, UserRole.SocialsDesigner, UserRole.GraphicDesigner, UserRole.Marketing] },
];
  ...
  const SidebarContent = () => (
    ...
    <div className="flex flex-col flex-grow">
      <div className="p-6 flex items-center mb-2">
        <div className="bg-gradient-to-br from-brand-surface to-brand-dark-secondary p-2 rounded-xl mr-3 border border-brand-border/50 shadow-lg">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 group ${
                isActive 
                ? 'bg-brand-accent/10 text-brand-accent border border-brand-accent/20 shadow-[0_0_15px_-5px_rgba(14,165,233,0.3)]' 
        ...
        <div className="pt-4 mt-4 border-t border-brand-border/50">
            ...
            {user?.role === UserRole.Manager && (
                ...
                >
                    ...
                    <span className="ml-3">Manager Panel</span>
                </NavLink>
            )}
             <NavLink
                to="settings"
      ...
      <div className="p-4 m-3 rounded-xl bg-gradient-to-b from-white/5 to-transparent border border-white/5 backdrop-blur-sm">
        <div className="flex items-center mb-3">
          ...
          <div className="ml-3 overflow-hidden">
            ...
            <p className="text-[10px] text-brand-text-secondary uppercase tracking-wider truncate">{user?.role}</p>
          </div>
        </div>
        <button
          onClick={logout}
